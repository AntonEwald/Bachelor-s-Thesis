...
colnames(Summary_LOQ30_Slope1.01_neg) <- c("Individual Sd", "Random Effect", "Method", "$(\\hat{\\beta} - \\beta)^2$", "Coverage", "Var($\\hat{\\beta}$)", "MSE", "Se($\\hat{\\beta}$)")
...
colnames(Summary_LOQ30_Slope1.05_neg) <- c("Individual Sd", "Random Effect", "Method", "$(\\hat{\\beta} - \\beta)^2$", "Coverage", "Var($\\hat{\\beta}$)", "MSE", "Se($\\hat{\\beta}$)")
...
colnames(Summary_LOQ60_Slope1.01_neg) <- c("Individual Sd", "Random Effect", "Method", "$(\\hat{\\beta} - \\beta)^2$", "Coverage", "Var($\\hat{\\beta}$)", "MSE", "Se($\\hat{\\beta}$)")
...
Plot_High_Individual_Variance_neg <- betas_spread_individual_high_neg %>% ggplot(aes(x=Beta, y=Museum_Beta, col = `Random Effects`, shape = `Random Effects`)) +
  ...
  labs(caption = "Figure 8: Plotting the estimated slopes for the substitution method and the LMMC model against eachother having the variance of individual specimens 
...
Plot_Low_Individual_Variance_neg <- betas_spread_individual_low_neg %>% ggplot(aes(x=Beta, y=Museum_Beta, col = `Random Effects`, shape = `Random Effects`)) +
  ...
  geom_hline(data = data.frame(Slope_Perc="5% yearly decrease", y=log(0.95)), mapping=aes(yintercept=y))+
   geom_vline(data = data.frame(Slope_Perc="5% yearly decrease", x=log(0.95)), mapping=aes(xintercept=x)) +
  labs(caption = "Figure 6: Plotting the estimated slopes  for the substitution method and the LMMC modelagainst eachother having the variance of individual specimens
...
Plot_Medium_Individual_Variance_neg <- betas_spread_individual_medium_neg %>% ggplot(aes(x=Beta, y=Museum_Beta, col = `Random Effects`, shape = `Random Effects`)) +
  ...
  geom_hline(data = data.frame(Slope_Perc="5% yearly decrease", y=log(0.95)), mapping=aes(yintercept=y))+
   geom_vline(data = data.frame(Slope_Perc="5% yearly decrease", x=log(0.95)), mapping=aes(xintercept=x)) +
  labs(caption = "Figure 7: Plotting the estimated slopes for the substitution method and the LMMC model against eachother having the variance of individual specimens 

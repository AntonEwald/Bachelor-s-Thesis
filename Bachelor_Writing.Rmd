...
When taking a closer look at the coverage of both methods the Tobit model clearly has a coverage over $95 \%$ in all cases but one (see Table 4). On the other hand, even though using substitution might have produced less bias in most cases, the coverage is not even close to the promised $95 \%$. Anytime the errorterms are held at a low level, the coverage is close to zero. There are at the same time no clear patterns for any of the factors affecting the coverage of the Tobit model except for a slight decrease in coverage when the between-year errors are set to low over high. 

...
betas <- Completed_df_New %>% filter(Method != "True") %>% mutate(Slope_Perc = ifelse(Slope < 0.01, "1%", "5%"))
...
Plot_High_Individual_Variance <- betas_spread_individual_high %>% ggplot(aes(x=Beta, y=Museum_Beta, col = `Random Effects`)) +
  facet_grid(Slope_Perc ~ `Limit fraction`) +
  ...
  and Tobit model against eachother having the variance of individual specimens set to high.
...
Plot_Low_Individual_Variance <- betas_spread_individual_low %>% ggplot(aes(x=Beta, y=Museum_Beta, col = `Random Effects`)) +
  facet_grid(Slope_Perc ~ `Limit fraction`) +
  geom_point(alpha = 0.6) + 
   geom_hline(data = data.frame(Slope_Perc="1%", y=log(1.01)), mapping=aes(yintercept=y)) +
   geom_vline(data = data.frame(Slope_Perc="1%", x=log(1.01)), mapping=aes(xintercept=x)) +
  geom_hline(data = data.frame(Slope_Perc="5%", y=log(1.05)), mapping=aes(yintercept=y))+
   geom_vline(data = data.frame(Slope_Perc="5%", x=log(1.05)), mapping=aes(xintercept=x)) +
  labs(caption = "Figure 2: Plotting the estimated slopes for the Museum model
  and Tobit model against eachother having the variance of individual specimens set to low.
  The vertical and horizontal lines correspond to the true value of the slope.")
...
Plot_Medium_Individual_Variance <- betas_spread_individual_medium %>% ggplot(aes(x=Beta, y=Museum_Beta, col = `Random Effects`)) +
  facet_grid(Slope_Perc ~ `Limit fraction`) +
  geom_point(alpha = 0.6) + 
   geom_hline(data = data.frame(Slope_Perc="1%", y=log(1.01)), mapping=aes(yintercept=y)) +
   geom_vline(data = data.frame(Slope_Perc="1%", x=log(1.01)), mapping=aes(xintercept=x)) +
